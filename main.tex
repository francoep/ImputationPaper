...
In particular, we demonstrate that utilizing imputed labels from taken from a convolutional neural network trained only on the affinity data present in CrossDocked2020 results in a small improvement on binding affinity regression performance, despite the additional sources of noise that such imputed labels add to the training data.
